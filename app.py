...
mydb=mysql.connector.connect(host="localhost",user="sumanth",password="Admin",db='event_chat')
...
def vsignup():
    if request.method=='POST':
        ...
        try:
            ...
            data={'vendor':vendor,'mobile':mobile,'email':email,'address':address,'password':password,'services':services}
            subject='The Confirmation Link For Event Planning Website'
...
def vconfirm(token):
    try:
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('insert into vendors(name,mobile,email,address,password,services) values(%s,%s,%s,%s,%s,%s)',[data['vendor'],data['mobile'],data['email'],data['address'],data['password'],data['services']])
        mydb.commit()
        cursor.close()
        flash('Your details has registered successfully')
        return redirect(url_for('vlogin'))
...
def profile():
    if session.get('vendor'):
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select vendor_id,name,mobile,email,services from vendors where vendor_id=(select vendor_id from vendors where name=%s)',[session.get('vendor')])
...
def vendor_tasks():
    if session.get('vendor'):
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select t.task_id,t.event_id,t.title,t.assigned_to,t.user_id,b.budget_id,b.total_budget,u.username from tasks t join budgets b on t.task_id=b.task_id join users u on t.user_id=u.user_id')
        data=cursor.fetchall()
        print(data)
        cursor.close()
